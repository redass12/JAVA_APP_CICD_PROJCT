pipeline{
    ...
    stages{
        ...
        stage('identifier les misconfigs en utilisant datree in helm charts'){
            steps{
                script{
        ...
        stage("le pushing de helm charts vers nexus"){
            steps{
                script{
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                          dir('kubernetes/') {
    ...
    post {
		always {
			mail bcc: '', body: "<br>Projet: ${env.JOB_NAME} <br>Build Number: ${env.BUILD_NUMBER} <br> URL de build: ${env.BUILD_URL}", cc: '', charset: 'UTF-8', from: '', mimeType: 'text/html', replyTo: '', subject: "${currentBuild.result} CI: Project name -> ${env.JOB_NAME}", to: "redataraoui@gmail.com";  
		 }
	   }
}
